# AGENTS.md — 수신호(Mulsigye) 하네스 진입점
...
**수신호** — 농업용수 부족 시점을 예측하고, 농업인에게 지금 해야 할 행동을
...
5. **가뭄단계 임계값은 공인 기준만 사용.** 평년 대비 70/60/50/40% (관심/주의/경계/심각).
   임계값이나 새 위험 등급 체계를 자체로 만들지 않는다.
   **예외(2026-07-26 승인): 공표 자료가 없는 날짜의 `avgRatio` 추정.** 공표 논가뭄지도가 연 1회만
   갱신돼 오늘 값이 없으므로, 저수지 단위 실측을 공사의 통합저수율 정의(유효저수량 가중 평균)로
   집계해 같은 임계값을 적용한 **추정치**를 쓸 수 있다. 다만 아래를 모두 지킨다.
   - 산출물 `data/snapshots/region-estimator.json`으로만 계산하고, 구간을 **학습/검증/시험**
     3분할해 지역별 오차를 검증한다(`pnpm --filter @mulsigye/web build:estimator`).
   - **게이트는 학습에 쓰지 않은 검증 구간**에서 판정하고(2.0%p 초과 시 **공표값으로 폴백**),
     화면·문서에 밝히는 오차는 그 판정에도 쓰지 않은 **시험 구간** 값이다.
   - 화면·응답에 **추정치임과 기준일**을 반드시 밝히고, 공표값이 있는 날짜는 공표값이 우선이다.
   - 임계값(70/60/50/40)은 그대로 쓴다 — 바뀌는 것은 입력값의 출처뿐이다.
