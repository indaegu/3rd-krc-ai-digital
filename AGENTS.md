...
5. **가뭄단계 임계값은 공인 기준만 사용.** 평년 대비 70/60/50/40% (관심/주의/경계/심각).
   ...
   **예외(2026-07-26 승인): 공표 자료가 없는 날짜의 `avgRatio` 추정.** 공표 논가뭄지도가 연 1회만
   ...
   - 산출물 `data/snapshots/region-estimator.json`으로만 계산하고, 학습/검증 구간을 나눠
     지역별 오차를 검증한다(`pnpm --filter @mulsigye/web build:estimator`).
   - 학습 구간 MAE가 게이트(2.0%p)를 넘는 지역은 추정하지 않고 **공표값으로 폴백**한다.
