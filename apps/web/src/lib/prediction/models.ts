// avgRatio 30일 예측 모델 5종 — 결정적 순수 함수(Date.now/네트워크/랜덤 금지).
// 입력은 날짜 오름차순 avgRatio 시계열(number[]), 출력은 30일 예측 number[].
...
/**
 * 예측 지평(일). forecast[29]가 30일 뒤 값이다.
 * 앞뒤로 최소 한 달씩 보이게 해달라는 현장 요구로 14 → 30으로 늘렸다. 지평이 길어질수록
 * 잔차 분위수 밴드가 넓어지므로, 불확실성은 밴드로 그대로 드러난다(단정 표현 금지 규칙 3).
 */
 ...
 * 예측 지평과는 무관하다 — 지평을 늘린다고 더 긴 입력을 요구하지 않는다.
 */
export const MODEL_MIN_INPUT_DAYS: Record<PredictionModelName, number> = {
  naive: LINEAR_WINDOW_DAYS,
  ma7: LINEAR_WINDOW_DAYS,
  linear: LINEAR_WINDOW_DAYS,
  ses: LINEAR_WINDOW_DAYS,
  damped: LINEAR_WINDOW_DAYS,
};

/** 모델 이름으로 30일 예측을 낸다. 입력이 최소 길이(14일) 미만이면 에러. */
...
/**
 * 일일 변화량 d(%p/day)의 예측 기반 정의: (forecast의 마지막 - r0) / 지평일수.
