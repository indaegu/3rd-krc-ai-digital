...
fun ReservoirGauge(
    ...
    Canvas(
        ...
        if (fillFraction > 0f) {
            ...
            clipPath(container) {
                if (waterTop <= amplitude * 2f) {
                    // 만수(평년 대비 100% 이상)에서는 물결을 그리지 않고 통을 가득 채운다.
                    // 수면이 통 맨 위에 붙으면 사인 골이 통 밖으로 잘려 상단에 흰 홈이 생겼다.
                    drawRect(color = Blue, size = Size(beakerRight, h))
                } else {
                    // 물 색 = 브랜드 파랑 고정. 뒤 물결은 옅은 파랑, 앞 물결은 파랑으로 살짝 물결진다.
                    drawWave(surfaceY = waterTop, phase = waveB, amplitude = amplitude, color = Blue.copy(alpha = 0.5f), width = beakerRight, height = h)
                    drawWave(surfaceY = waterTop + amplitude, phase = waveA, amplitude = amplitude, color = Blue, width = beakerRight, height = h)
                }
            }
        }
