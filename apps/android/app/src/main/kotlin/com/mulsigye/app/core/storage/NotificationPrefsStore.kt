...
class NotificationPrefsStore(
    ...
    /** 마지막으로 알린 단계 코드 저장(기준선 갱신). null이면 키를 삭제한다. */
    suspend fun setLastNotifiedStageCode(code: String?) = dataStore.edit {
        if (code == null) it.remove(LAST_STAGE) else it[LAST_STAGE] = code
    }
