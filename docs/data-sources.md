...
**GPS 거리나 좌표 기반 “가장 가까운 저수지”는 계산하지 않는다.** 시설제원 CSV에는 좌표 열이
없어(수혜면적·유효저수량뿐) 거리를 계산할 근거가 없다 — 지어내지 않는다.

대신 도로명주소가 주는 **읍·면·동(`emdNm`)·리(`liNm`)** 로 후보를 좁힌다(2026-07-26 변경).
시군 하나만 보면 넓은 시군에서 엉뚱한 저수지가 나왔다 — 실측: 제주시(50110)는 한림읍·조천읍·
구좌읍에 걸쳐 저수지가 5곳인데 수혜면적 최대(상대 670ha, 한림읍)가 늘 뽑혀, 조천읍 주소를
골라도 한림읍 상대 저수지가 대표지였다.

1. 도로명주소 API에서 시군구 코드와 읍·면·동·리 이름을 얻는다.
2. 주소 원문(도로명·번지)은 대표지 결정 후 폐기하며 로그·Supabase·분석 도구에 저장하지 않는다.
   읍·면·동·리 이름도 대표지 결정에만 쓰고 저장하지 않는다.
3. 시설제원에서 같은 시군구 코드의 저수지만 후보로 둔다.
4. 후보를 **좁은 단위부터** 본다 — ① 같은 리 → ② 같은 읍·면·동 → ③ 같은 시군.
   후보가 있는 첫 단계에서 멈춘다. 소재지는 공백으로 끊은 **토큰 정확 일치**로 비교한다
   (“동명동”이 “동명리”에 걸리지 않게).
5. 그 단계 안에서 `beneficiaryArea`가 가장 큰 시설을 고른다.
6. 수혜면적이 같으면 `facCode` 문자열 오름차순의 첫 시설을 고른다.
7. 후보가 없으면 자동으로 인접 시군을 고르지 않고 “이 지역은 준비 중이에요”를 반환한다.

**사용자가 저수지 이름으로 직접 고른 경우**(`GET /api/v1/reservoirs/search` → `facCode`)에는
위 규칙을 건너뛰고 그 시설을 대표지로 쓴다. 같은 시군 후보일 때만이며, 아니면 무시하고 규칙대로
고른다. 선택은 기기에 `facCode`로 저장되고 `GET /api/v1/status?facCode=`에 실려 유지된다 —
그러지 않으면 매 요청마다 규칙이 선택을 덮어 검색 기능이 무의미해진다.

API와 UI의 명칭은 항상 **“우리 지역 대표 저수지”**다. 거리 필드는 계약에 넣지 않으며
“가까운/가장 가까운”이라는 표현도 쓰지 않는다(거리를 계산하지 않으므로 사실이 아니다).
