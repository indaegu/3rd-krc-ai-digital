...
- 보정계수 `k_r`는 학습 구간(2025-01-01~09-30)의 `공표 저수율 / 원시 추정` 중앙값이며,
  보정이 오히려 나쁜 지역은 1.0을 쓴다.

### 검증(학습에 쓰지 않은 2025-10-01~12-31)

아래 수치는 모두 산출물 `region-estimator.json`의 `summary`에서 그대로 옮긴 값이며,
게이트를 통과한 지역·검증 구간 표본(8,096건)에서만 계산한다.

| 항목 | 값 |
|---|---|
| 평가 지역 | 114곳 중 **게이트 통과 88곳** |
| 평년 대비 MAE | **1.05%p** (중앙값 0.51, p90 2.71) |
| 공인 단계 일치율 | **99.5%** |
| 모호 조인 제외 | 2쌍 |
