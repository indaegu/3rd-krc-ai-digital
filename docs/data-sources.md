...
산출물은 `apps/web/src/lib/data/region-estimate.ts` 한 곳에서만 읽는다. `/api/v1/status`는
지역 값을 확정할 때 아래 순서를 지킨다.

1. 공표 지역 값(`regional_drought_daily` → 커밋 스냅샷)을 먼저 확보한다.
2. 게이트 통과 지역이면 수위 API **시군 조회**(`county={시군명}`, 최근 7일)로 실측을 모은다.
   `county`는 코드가 아니라 이름을 받는다(실측: `50110`은 NO_DATA, `제주시`는 정상).
3. 최신 날짜부터 뒤로 물러나며, 그 날의 관측이 시군 유효저수량의 **80% 이상**을 덮는
   첫 날짜로 통합저수율 → 평년 대비를 계산한다. 못 덮으면 계산하지 않는다.
4. 계산된 날짜가 공표 기준일보다 **최신일 때만** 추정을 쓴다(공표값 우선 원칙).
5. 조회 실패·커버리지 미달·게이트 탈락은 전부 공표값 폴백이며, 응답이 달라지지 않는다.
...
집계 저수지 수, 용량 비중)를 함께 싣는다. 화면은 이 값을 그대로 표기한다 — 웹·Android 모두
"오늘 추정" 배지와 근거 카드 문구를 붙이며, 단계 임계값(70/60/50/40)은 공인 기준 그대로다.
