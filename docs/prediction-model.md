...
## 감소 주의 조기경보 (앱 자체 참고 신호 — 공식 단계 아님)

공식 가뭄 단계는 **평년 대비 저수율(avgRatio)의 절대 수준** 70/60/50/40으로만
판정한다(위 "다음 단계 도달 가능 시점"과 `drought-stage.ts`가 단일 출처). 그런데
어떤 지역은 아직 정상·관심이어도 **빠르게 줄고 있을** 수 있다. 이 하락 속도를
공식 단계와 별개로 미리 알리는 것이 조기경보다.

- **정의**: 이미 계산한 관측 기울기 `trend.dailyDelta`(=`observedDailyDelta`,
  최근 14일 OLS 기울기)를 재사용한다. 새 산식·새 시계열을 만들지 않는다.
- **임계값(앱 자체)**: `EARLY_WARNING_DAILY_DROP = -0.7 %p/day`
  (≈ -5%p/주, 자문 "2안"의 -5%p/7일과 정합). `models.ts`의 다른 예측 상수와 같은
  자리에 두고 **공식 가뭄 기준이 아님**을 주석으로 명시한다. 70/60/50/40을
  복제하지 않으며, 이 상수는 공식 단계 판정에 영향을 주지 않는다.
- **게이팅**: 현재 공식 단계가 **정상·관심(ok/watch)** 일 때만 켠다. 이미 주의·경계·
  심각이면(더 나쁜 단계) 조기경보를 내지 않는다(그 단계는 이미 공식 알림 대상).
  조건: `officialStage ∈ {ok, watch}` **그리고** `trend.dailyDelta ≤ -0.7`.
- **출력**: `earlyWarning: { level: "watch", dailyDelta, message } | null`.
  `level`은 현재 `watch`만이며 향후 `urgent` 확장 여지로 객체로 둔다. 조건 미충족이면
  `null`이다. `message`는 ~해요체 참고 문구이며 "공식 단계와 별개인 참고 신호"임을
  문장에 담는다(확정 표현 금지, 규칙 3).
- **표시**: 웹·Android 모두 공식 단계 칩과 **구분되는** watch 톤 배너로 ReachCard
  근처에 보여준다. 공식 단계를 대체·재판정하지 않는 **참고 신호**이며, 값이 없으면
  아무것도 그리지 않는다(비차단, additive). 서버가 확정하고 클라이언트는
  `earlyWarning.message`를 그대로 쓴다(자체 임계값 복제 금지, 규칙 10).
...
- [x] `data/backtest-report.json` 커밋 — 채택 모델의 horizon 1~~14 잔차 경험적
      p25/p75(50% 구간) 포함(예: h7 −1.0~~+0.7, h14 −2.0~+1.2, horizon당 924표본).
      지역별 `bandScale`(지역 rmse14 / macro rmse14 5.1504, 0.6–1.6 클램프)도 함께 기록한다
      (예: 44230 논산 1.6, 46170 나주 0.87). 밴드 산식 메타데이터는
