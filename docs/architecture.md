...
- 백그라운드 알림(`WaterCheckWorker`)은 `StatusRepository.load(allowCached = false)`로 부른다.
  저장본으로 알림을 보내면 며칠 전 물 사정을 오늘 일처럼 알리고, 단계 기준선까지 덮어써
  나중에 실제로 나빠졌을 때의 알림을 삼킨다. 저장본은 사람이 보고 있는 화면에서만 쓴다.
- 저장본 읽기·쓰기 실패는 화면에 영향을 주지 않는다. 쓰기 실패가 방금 받은 정상 응답을
  버리거나, 읽기 실패가 예외로 새어 나가 로드 코루틴을 죽이지 않도록 각각 격리한다.
