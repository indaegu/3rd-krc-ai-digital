...
Vercel 프로젝트, Android application ID·서명키·배포 계정, LLM 제공자 결정, 저작재산권 문의.

**완료:** `.env.local`에 필요한 키가 있고 `.env.example`에는 이름만 있으며, Android 서명키가
저장소 밖에 백업되어 있다. LLM이 미결정이어도 정적 코치 폴백으로 단계 3까지 진행할 수 있다.

### 1. 저장소 스캐폴드·공용 계약

**산출물:** Next.js 앱, `contracts/openapi.yaml`, Android Compose 앱,
`supabase/migrations/`, CI, 실제 검증 명령.

**완료:** `/api/v1/health`가 OpenAPI 계약과 일치하고 웹·Android에서 호출된다.
`docs/testing-and-feedback.md`의 `스캐폴드 대기` 명령을 실제 실행해 `동작`으로 바꾼다.
...
| 07-22 | LLM 제공자·모델 확정 | 비용·지연·출력 JSON 비교 기록 |
