...
- 지역 목록 관리 모드: 길게 누름/“지역 관리” 진입 → 드래그 순서 변경 + 체크박스 다중 삭제.
  드래그는 “위로/아래로 이동” 커스텀 액션으로 접근성 대체를 제공한다.
...
- 알림은 **명시적 옵트인 설정**만 허용한다(Android 전용, 기본 꺼짐). 지역 설정 화면에 중립적인 “알림 설정”
  진입만 두고, 켜라고 부추기는 유도(다크패턴) 문구·배너·상시 배지는 만들지 않는다. 웹에는 알림 CTA가 없다.
